...
Пользователь регистрируется в системе, указывая login и пароль. После авторизации пользователь получает доступ к персонализированным функциям.
...
request (хранит историю запросов пользователей):
...
weather (содержит данные о текущей погоде):
...
days (хранит прогноз погоды на несколько дней):
...
2. Запрашивает погоду для города "Москва".
3. Проверяет, что запрос сохранен.
